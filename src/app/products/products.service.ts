...
export class ProductsService {
  ...
  public getProducts(query: any): Observable<any> {
    const whishlist: Product[] = JSON.parse(
      this._localStorage.getItem('wishlistData')
    );
    ...
    return this.httpClientService
      ...
      .pipe(
        map((res: Product[]) => {
          res.map((product) => {
            product.isLiked = !!whishlist?.find(
              (wishlistItem) => wishlistItem.id == product.id
            );
          });
          // res = res.filter(x => x.name.toLowerCase().includes('clean'))
          let products: Product[] = res.slice(
  ...
  public getProduct(productId: number): Observable<Product> {
    return this.httpClientService
      .get<any>(
        `http://makeup-api.herokuapp.com/api/v1/products.json?product_category=powder&id=${productId}`
      )
      .pipe(
        map((res: Product[]) => {
